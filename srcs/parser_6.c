...
/*   Updated: 2020/02/03 23:37:34 by abobas        ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void	dimension_x(t_map *map, char *map_str)
{
	...
	while (y < map->map_y)
	{
		...
		while (map_str[i] != '\n' && map_str[i] != '\0')
		{
			if ((map_str[i] >= '0' && map_str[i] <= '2') || map_str[i] == 'N' \
...
void	fill_row(t_map *map, char *map_str, int y)
{
	...
	while (x < map->map_x[y])
	{
		if (map_str[map->index] >= '0' && map_str[map->index] <= '2')
		{
